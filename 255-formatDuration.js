...
const getTime = (seconds, time) => seconds >= time ? parseInt(seconds / time) : 0;
...
const formatDuration = (seconds) => {
    ...
    for(let i in times) {
        const time = getTime(seconds, times[i]);
        seconds -= (times[i] * time);

        if(time > 0) {
            result.push(`${time} ${units[i]}${time > 1 ? 's' : ''}`);
        }
    }

    return `${result.length > 2 ? result.slice(0, result.length - 2).join(', ') + ', ' : ''}${result.slice(result.length - 2).join(' and ')}`;
}
